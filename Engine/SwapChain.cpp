...
SwapChain::SwapChain(HWND hwnd, ui32  width, ui32  height, RenderSystem* system) : m_system(system){
	...
	if (FAILED(hr)) {
		throw std::exception("SwapChain did not initiate successfully");
	}
	...
	if (FAILED(hr)) {
		throw std::exception("Texture2D did not initiate successfully");
	}
...
void SwapChain::resize(ui32 width, ui32 height) {
	...
	if (FAILED(hr)) {
		throw std::exception("SwapChain resize failed");
	}
	...
	// recreate the depth/stencil buffer at the new size too
	D3D11_TEXTURE2D_DESC tex_desc = {};
	tex_desc.Width = width;
	tex_desc.Height = height;
	tex_desc.Format = DXGI_FORMAT_D24_UNORM_S8_UINT;
